...
@router.get("/price")
async def price_endpoint(game: str, item: str, currency:str):
    data = await price(game, item, currency)
    return data
...
async def sales_history_endpoint(
    ...
    data = await sales(game, item, period, currency)
    return data
...
async def markets_prices_endpoint(game: str, item:str, currency:str):
    data = await markets_prices(game, item, currency)
    return data
